...
class LanguageModel(torch.nn.Module):
    ...
    def batch_nll(self, sentences, prefix):
        ...
        if not prefix:
            o = torch.cat([h0[0][0].unsqueeze(1), o], dim=1)
            target = torch.cat([input[:, 0].view(-1, 1), target], dim=1)
            mask = torch.cat([torch.ones((batch_size, 1), dtype=torch.int64), mask], dim=1)
