...
echo "*** Building model"
echo "******************"
python $BIN_DIR/model_building/build_bengio_ivec_input.py \
...
python $BIN_DIR/train/train-ivecs-oracle.py \
...
do
    python $BIN_DIR/eval/eval-ivecs-partial.py \
...
for TESTLIST in train-list.txt valid-list.txt test-list.txt
do
    python $BIN_DIR/eval/eval-ivecs-domain-adaptation.py \
        --file-list=$DATA_ROOT/$TESTLIST \
        --cuda \
        --batch-size=20 \
        --domain-portion=0.25 \
